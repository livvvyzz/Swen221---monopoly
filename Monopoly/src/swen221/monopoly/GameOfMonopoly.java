...
public class GameOfMonopoly {
	...
	public void unmortgageProperty(Player player, Location loc) throws InvalidMove {
		...
		if (!prop.isMortgaged()) {
			throw new InvalidMove(player + " cannot unmortgage location " + loc.getName() + ": it's not mortgaged!");
		}
	...
	public void buildHotel(Player player, Location loc) throws InvalidMove {
		if (!(loc instanceof Street)) {
			throw new InvalidMove(player + " cannot build hotel on location " + loc.getName() + ": it's not a street!");
		}
		// Check street is not mortgaged
		Street street = (Street) loc;
		if (street.isMortgaged()) {
		...
		if (street.getHotels() != 0) {
			...
			throw new InvalidMove(
					player + " cannot build hotel on location " + loc.getName() + ": need five houses first!");
		}
		// Check available funds
